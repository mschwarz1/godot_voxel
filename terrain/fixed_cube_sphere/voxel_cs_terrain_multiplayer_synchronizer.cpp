...
// Helper function
bool VoxelCSTerrainMultiplayerSynchronizer::is_server() const {
...
void VoxelCSTerrainMultiplayerSynchronizer::send_block(
...
void VoxelCSTerrainMultiplayerSynchronizer::send_area(Box3i voxel_box) {
	ZN_PROFILE_SCOPE();
...
void VoxelCSTerrainMultiplayerSynchronizer::_notification(int p_what) {
	if (p_what == NOTIFICATION_PARENTED) {
		VoxelCubeSphereTerrain *terrain = Object::cast_to<VoxelCubeSphereTerrain>(get_parent());
...
void VoxelCSTerrainMultiplayerSynchronizer::process() {
	ZN_PROFILE_SCOPE();
			...
			if (terrain != nullptr && terrain->get_multiplayer_synchronizer() != this) {
				warnings.append(ZN_TTR("Only one instance of {0} should exist under a {1}")
										.format(varray(VoxelCSTerrainMultiplayerSynchronizer::get_class_static(),
												VoxelCubeSphereTerrain::get_class_static())));
			}
		}
	}
}
...
void VoxelCSTerrainMultiplayerSynchronizer::_bind_methods() {
	// TODO These methods are not supposed to be exposed. They only exist for Godot's high-level multiplayer to find
	// them.
	ClassDB::bind_method(
			D_METHOD("_rpc_receive_blocks", "data"), &VoxelCSTerrainMultiplayerSynchronizer::_b_receive_blocks);
	ClassDB::bind_method(D_METHOD("_rpc_receive_area", "data"), &VoxelCSTerrainMultiplayerSynchronizer::_b_receive_area);
}
