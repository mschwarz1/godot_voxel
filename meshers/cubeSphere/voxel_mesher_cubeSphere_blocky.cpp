		...
		float baked_occlusion_darkness, int curveRes, int heightRes, Vector3i origin) 
{
	...
	ERR_FAIL_COND(block_size.x < static_cast<int>(2 * VoxelMesherCubeSphereBlocky::PADDING) ||
			block_size.y < static_cast<int>(2 * VoxelMesherCubeSphereBlocky::PADDING) ||
			block_size.z < static_cast<int>(2 * VoxelMesherCubeSphereBlocky::PADDING));
			...
			for (unsigned int y = min.y; y < (unsigned int)max.y; ++y) {
				...
				for (unsigned int side = 0; side < Cube::SIDE_COUNT; ++side) {
					...
					for (unsigned int surface_index = 0; surface_index < model.surface_count; ++surface_index) {
						...
						VoxelMesherCubeSphereBlocky::Arrays &arrays = out_arrays_per_material[surface.material_id];
...

						{
							...
							Vector3f p0 = sideVertPositions[side_indices[0]];
							Vector3f p1 = sideVertPositions[side_indices[1]];
...
void VoxelMesherCubeSphereBlocky::set_curvature(int newRes) {
	RWLockWrite wlock(_parameters_lock);
	curveRes = newRes;
}

int VoxelMesherCubeSphereBlocky::get_curvature() {
	RWLockRead rlock(_parameters_lock);
	return curveRes;
}
