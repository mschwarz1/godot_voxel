#include "voxel_tool_cube_sphere_terrain.h"
...
VoxelToolCubeSphereTerrain::VoxelToolCubeSphereTerrain() {
	_random.randomize();
}

VoxelToolCubeSphereTerrain::VoxelToolCubeSphereTerrain(VoxelCubeSphereTerrain *terrain) {
	ERR_FAIL_COND(terrain == nullptr);
	_terrain = terrain;
	// Don't destroy the terrain while a voxel tool still references it
}

bool VoxelToolCubeSphereTerrain::is_area_editable(const Box3i &box) const {
	ERR_FAIL_COND_V(_terrain == nullptr, false);
	// TODO Take volume bounds into account
	return _terrain->get_storage().is_area_loaded(box);
}

Ref<VoxelRaycastResult> VoxelToolCubeSphereTerrain::raycast(
...
void VoxelToolCubeSphereTerrain::copy(Vector3i pos, VoxelBuffer &dst, uint8_t channels_mask) const {
...
void VoxelToolCubeSphereTerrain::paste(Vector3i pos, const VoxelBuffer &src, uint8_t channels_mask) {
	ERR_FAIL_COND(_terrain == nullptr);
	if (channels_mask == 0) {
		channels_mask = (1 << _channel);
	}
...
void VoxelToolCubeSphereTerrain::paste_masked(Vector3i pos, Ref<godot::VoxelBuffer> p_voxels, uint8_t channels_mask,
...
void VoxelToolCubeSphereTerrain::do_sphere(Vector3 center, float radius) {
	ZN_PROFILE_SCOPE();
	ERR_FAIL_COND(_terrain == nullptr);
...
void VoxelToolCubeSphereTerrain::do_hemisphere(Vector3 center, float radius, Vector3 flat_direction, float smoothness) {
	ZN_PROFILE_SCOPE();
	ERR_FAIL_COND(_terrain == nullptr);
...
uint64_t VoxelToolCubeSphereTerrain::_get_voxel(Vector3i pos) const {
...
float VoxelToolCubeSphereTerrain::_get_voxel_f(Vector3i pos) const {
	ERR_FAIL_COND_V(_terrain == nullptr, 0);
	return _terrain->get_storage().get_voxel_f(pos, _channel);
}

void VoxelToolCubeSphereTerrain::_set_voxel(Vector3i pos, uint64_t v) {
	ERR_FAIL_COND(_terrain == nullptr);
	_terrain->get_storage().try_set_voxel(v, pos, _channel);
}

void VoxelToolCubeSphereTerrain::_set_voxel_f(Vector3i pos, float v) {
	ERR_FAIL_COND(_terrain == nullptr);
...
Variant VoxelToolCubeSphereTerrain::get_voxel_metadata(Vector3i pos) const {
	ERR_FAIL_COND_V(_terrain == nullptr, Variant());
	VoxelData &data = _terrain->get_storage();
	return data.get_voxel_metadata(pos);
}

void VoxelToolCubeSphereTerrain::run_blocky_random_tick_static(VoxelData &data, Box3i voxel_box,
...
namespace {
Ref<VoxelBlockyLibraryBase> get_voxel_library(const VoxelCubeSphereTerrain &terrain) {
...
void VoxelToolCubeSphereTerrain::run_blocky_random_tick(
	...
	ZN_PRINT_ERROR("VoxelToolCubeSphereTerrain::run_blocky_random_tick isn't supported in GDExtension, cannot call Callables");
#endif
}

void VoxelToolCubeSphereTerrain::for_each_voxel_metadata_in_area(AABB voxel_area, const Callable &callback) {
	ERR_FAIL_COND(_terrain == nullptr);
	ERR_FAIL_COND(callback.is_null());
	ERR_FAIL_COND(!math::is_valid_size(voxel_area.size));
...
#elif defined(ZN_GODOT_EXTENSION)
		// TODO GDX: Can't call Callables
		ZN_PRINT_ERROR("VoxelToolCubeSphereTerrain::for_each_voxel_metadata_in_area isn't supported in GDExtension, cannot call "
					   "Callables");
#endif
	});
}

void VoxelToolCubeSphereTerrain::do_path(Span<const Vector3> positions, Span<const float> radii) {
	ZN_PROFILE_SCOPE();
